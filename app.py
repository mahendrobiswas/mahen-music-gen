...
# -*- coding: utf-8 -*-
# =====================================================================
#  MAHEN AI STUDIO  -  single-file AI Song + Image MAKER (Flask)
# ---------------------------------------------------------------------
#  Features:
...
# Vercel/Render compatible paths
...
# ---------- IMAGE ENDPOINTS (proxy to hazex workers) ----------
@app.get("/api/image")
def api_image():
...
# =====================================================================
#   MAHEN AI STUDIO — UI (MUSIC + IMAGE + LIBRARY)
# =====================================================================
_UI = r"""<!DOCTYPE html>
<html lang="en"><head>
<meta charset="utf-8">
...
.dockm{position:fixed;left:50%;bottom:calc(10px + var(--ios));transform:translateX(-50%);
 width:min(90vw,420px);background:rgba(20,18,30,.92);-webkit-backdrop-filter:blur(18px);backdrop-filter:blur(18px);
...
.dockm button{flex:1;border:0;background:transparent;height:46px;border-radius:19px;color:var(--mut);cursor:pointer;
 display:flex;align-items:center;justify-content:center;gap:5px;font-size:11.5px;font-weight:800}
.dockm button svg{width:16px;height:16px;fill:currentColor}
...
(function(){
  let dark;
  try{ dark = JSON.parse(localStorage.getItem('MAHEN_theme')||'null'); }
  catch(e){ dark=null; }
...
{Fore.WHITE}  LOGIC: {Fore.MAGENTA}  -> MUSIC + IMAGE + LIBRARY
{Fore.CYAN}{line}{Style.RESET_ALL}"""
    print(out)
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    print("  {0}Music + Image + Library ready — logs streaming below{1}\n".format(Y, X), flush=True)
    sys.stdout.flush()
